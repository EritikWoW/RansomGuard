...
0.7.6.0 retains the range-aware COW and rename reconciliation milestones and adds durable post-CREATE outcome reconciliation.
...
The engineering minifilter protocol is now v7 and reports CREATE, WRITE, RENAME, DELETE and TRUNCATE-class metadata operations. CREATE and RENAME now also produce correlated post-operation result events.
...
pre-images from data created during the same incident. For tracked CREATE dispositions, user mode also commits a hash-chained pre-operation intent linked to the required preservation record. After the filesystem completes the operation, the minifilter reconciles the tunneled final name and emits a correlated `CreateResult`; success, failure, or successful-but-unresolved completion is durably recorded separately. Missing result delivery leaves the CREATE intent pending rather than inferring success.
...
Remaining core work includes kernel file-ID binding of completed create/rename operations,
