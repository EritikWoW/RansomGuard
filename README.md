...
Crash/restart handling now quarantines any prior session with a durable CREATE/RENAME intent but no committed completion. LAB blocking startup is refused until that ambiguity is explicitly addressed; `--inspect-pending` reports exact request sequences without connecting to the driver or inferring an outcome from current files.

Remaining core work includes operator/verified reconciliation for quarantined pending CREATE/RENAME intents,
