...
0.7.20.0 retains the preservation, recovery and retention foundations and adds the first kernel-enforced LAB containment primitive.
...
The engineering minifilter protocol is now v12 and reports CREATE, WRITE, RENAME, DELETE and TRUNCATE-class metadata operations. RENAME events also carry a normalized destination path.
...
Protocol v12 now also treats CREATE as a two-phase durable transaction. Before allow, the gate records a
