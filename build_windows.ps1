...
try {
    ...
    if($IncludeLab){$projects+=@($sim,$filterClient,$gateClient,$runtimeHarness,$rollbackRecovery,$rollbackRetention)}
    ...
    if($IncludeLab){
        ...
        New-Item -ItemType Directory -Path $simDir,$clientDir,$gateClientDir,$runtimeHarnessDir,$rollbackRecoveryDir,$rollbackRetentionDir -Force | Out-Null
        ...
        Run-Dotnet -Arguments (@('publish',$rollbackRetention)+$publishFlags+@('-o',$rollbackRetentionDir))
    ...
    $unexpected=@(Get-ChildItem -LiteralPath $release -File -Recurse | Where-Object {
        ...
        $_.Name -like '*RuntimeHarness*' -or $_.Name -like '*RollbackRecovery*' -or $_.Name -like '*RollbackRetention*'
    })
