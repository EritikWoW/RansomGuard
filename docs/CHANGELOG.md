...
- Added the whole-session DeactivateGate maintenance command. GateClient requests it only after a clean, transaction-complete lifecycle terminal record; the kernel refuses deactivation while blocking gate sends or queued evidence work remain.
- Clean deactivation clears the containment latch and returns Maintenance before port close; faulted/incomplete shutdown does not authorize release and therefore remains fail-safe on disconnect.
