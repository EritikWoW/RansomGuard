...
The elevated setup path now has an explicit production-protection choice. Audit remains the default. Selecting production protection requires exactly one explicit local protected root and a complete version-bound `Protection/` package in the same reviewed release. The administrative trust boundary independently revalidates the service hash, descriptor, exact package layout, file digests, GateClient version, INF provider/altitude/DriverVer and any pre-existing production minifilter registration before it creates the service registration.

The selected package is copied into the new immutable service-version directory before registration, then re-read through the same package verifier used by updates. The generated configuration is `Mode=Enforce`, keeps the signed-driver requirement enabled and explicitly enables automatic containment. Missing or incompatible protection material is rejected before SCM registration. A partial pre-registration failure removes only the fresh immutable install directory and its known staged Protection files; it never recursively deletes an existing install tree.

Starting the service is still a separate reviewed action. SCM `Running` alone is not treated as protection evidence: the normal service lifecycle must independently reach `ProductionProtectionActivated` and `AutomaticContainmentReady`, otherwise startup remains fail-closed.
