...
The current LAB gate uses bounded kernel admission: at most 8 blocking gate sends may be outstanding. GateClient has a configurable 1..8 bounded message-processing slot limit (default 4), but reply-required preservation is intentionally serialized on the single synchronous Filter Manager handle so a worker completes FilterReplyMessage before the next blocking FilterGetMessage. No-reply completion/evidence events may overlap within the slot bound. Slow preservation no longer holds the global port mutex across the 30-second kernel wait.
...
0.7.28 validates bounded gate pressure and correlation under 20 concurrent helper processes. The kernel admission cap remains 8 and GateClient is configured with four bounded message-processing slots, while the single synchronous reply path remains serialized by design. Destructive length/delete requests are released only after their mutation-capable CREATE transactions are durably completed, then the run requires exact intent/completion request correlation, per-path TRUNCATE/DELETE binding, DELETE finalization, mapped pre-image SHA-256 and writable-section/paging evidence with no message-worker failures.


The next core milestones are crash/reboot and low-disk fault campaigns, Driver Verifier qualification, deeper rename/mapped-write soak testing, production retention UI/policy integration, directory-topology rollback semantics beyond startup handle exclusion, production detector-to-containment authorization/orchestration, process-state capture, adaptive crypto analysis, and production recovery UI/orchestration across rollback plus crypto evidence.
