...
0.7.13 also adds an activation barrier for state that predates the gate. The kernel starts each LAB connection as not activated; GateClient scans every existing file, and post-CREATE preflight checks `MmDoesFileHaveUserWritableReferences` against the real stream's section pointers. External protected-root mutations are fail-closed during this scan, while section/paging activity latches an activation hazard. The gate becomes active only after a successful explicit user-to-kernel `ActivateGate` handshake. Preflight evidence is durable and identity-bound.

The next core milestones are live VM proof of activation/section/paging ordering and remaining pre-existing delete-on-close/topology races, deeper crash recovery for requests interrupted before authoritative kernel completion delivery,
