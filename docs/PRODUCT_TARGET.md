...
0.7.11 adds conservative pre-preservation before a newly opened existing file can become a write-capable handle. FILE_OPEN/FILE_OPEN_IF with FILE_WRITE_DATA, GENERIC_WRITE, GENERIC_ALL or MAXIMUM_ALLOWED now commits the same identity-bound full-file pre-image used by destructive CREATE operations before allow. This gives writable file mappings created from handles opened while the LAB gate is active a pre-mutation recovery baseline without blocking the paging writer itself. Read-only opens retain the lower-cost no-preservation path. Legacy 0.7.10 CREATE journals remain readable, while new intents cannot record a write-capable existing-file open as unpreserved.

The next core milestones are writable-mapping coverage for handles that predate gate attachment, live mmap/cache-manager fault-injection validation, storage-pressure-aware optimization of eager full pre-images, deeper crash recovery for requests interrupted before authoritative kernel completion delivery,
