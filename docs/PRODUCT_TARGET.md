...
0.7.28 validates the bounded gate under mixed high concurrency. A disposable-VM runtime scenario fixes GateClient at four workers and drives 20 parallel CREATE/RENAME/TRUNCATE/DELETE/mapped-write helpers through the same protected root. Destructive length/delete requests are released only after their mutation-capable CREATE transactions are durably completed, then the run requires exact intent/completion request correlation, per-path TRUNCATE/DELETE binding, DELETE finalization, mapped pre-image SHA-256 and writable-section/paging evidence with no gate-worker failures.
