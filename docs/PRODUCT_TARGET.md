...
0.7.31 adds a dedicated long-duration/mixed-workload qualification contract without changing protocol v15. The accepted exact-head disposable-VM run kept one minifilter and one GateClient/rollback session active for 60 mixed waves with 10-second inter-round pauses (300 mixed mutations), while preserving exact transaction correlation, zero pending metadata transactions, hash-verified mapped pre-images, writable-section/paging evidence, worker health, elapsed-time coverage and bounded cleanup. Its admission-overflow probe also retained the kernel cap behavior of 16 requests producing 8 allowed / 8 denied.

0.7.32 adds a degraded-protected GateClient-loss qualification contract without changing protocol v15 structure sizes. An activated LAB gate that loses GateClient without the explicit maintenance-deactivation control preserves its negotiated root/mode in kernel `DEGRADED_PROTECTED` state. Resolved known in-scope mutation-capable CREATE, ordinary WRITE, RENAME/TRUNCATE/DELETE and creation of new writable sections on tracked streams fail closed; reconnect is refused so a replacement client cannot silently replace policy/root. Controlled shutdown drains the receive loop/workers and explicitly deactivates the gate first. The runtime qualification hard-stops GateClient, proves a protected-file mutation is denied with SHA-256 unchanged, proves reconnect rejection, then resets the LAB driver. Existing writable mappings remain recovery-covered rather than synchronously gated, and unresolved/name-query scope ambiguity remains a separate production blocker.

The next core milestones are production retention UI/policy integration, directory-topology rollback semantics beyond startup handle exclusion, production detector-to-containment authorization/orchestration, process-state capture, adaptive crypto analysis, and production recovery UI/orchestration across rollback plus crypto evidence.
