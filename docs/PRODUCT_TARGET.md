...
0.7.14 adds a dedicated runtime-validation harness rather than extending kernel policy again. A manual `workflow_dispatch` workflow can run only on a self-hosted disposable Windows VM labelled `ransomguard-lab-vm`. It builds the exact commit, test-signs the fresh driver, installs/attaches it, then verifies both pre-existing writable-mapping activation refusal and post-activation writable mapping with BaselineVerified section evidence, paging evidence and a byte-identical committed pre-image. Normal CI only validates the harness/source boundary and never loads the driver.

The next core milestones are obtaining repeatable live-VM evidence from the new harness across target Windows/filesystem variants, remaining pre-existing delete-on-close/topology races, deeper crash recovery for requests interrupted before authoritative kernel completion delivery,
