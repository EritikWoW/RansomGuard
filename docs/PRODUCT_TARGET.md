...
0.7.28 adds a dedicated bounded-concurrency runtime qualification without changing protocol v15. On the disposable self-hosted VM, GateClient runs at 8 workers while 16 independent helpers per phase exercise CREATE, RENAME, synchronized TRUNCATE, synchronized DELETE and mapped writes. The proof is not satisfied by helper exit codes alone: every targeted metadata transaction is matched back to unique durable intent/completion evidence, every delete requires pathname finalization, and every mapped target requires a hash-verified full pre-image plus writable-section and paging-write evidence.
