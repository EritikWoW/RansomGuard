...
The current engineering minifilter/GateClient wire contract is protocol v18. It retains protocol-v17 exact protected-volume scope and adds a distinct ProductionGate client profile. ProductionGate cannot use LAB-only containment reply flags, containment/query controls or scope-ambiguity fault injection; DegradedProtected reconnect also requires the retained gate profile in addition to the exact root/volume. The Engineering LAB driver package still uses an unassigned placeholder altitude and test-signing path and remains restricted to disposable test environments. ProductionGate wire separation is not equivalent to production driver lifecycle activation.
