...
Status: engineering threat model for RansomGuard 0.8.7 pre-enable, covering the default Audit product, admitted Production Enforce lifecycle, protocol-v18 LAB/ProductionGate separation, hard-link/FSCTL policy, the Engineering minifilter, and the qualified-but-still-unwired production containment actuator.
...
The normal service still does not actuate detector-driven containment for ordinary applications, including suspicious/canary cases. Audit mode stays non-blocking. Production Enforce can activate the admitted preservation lifecycle, but `AutomaticContainment=true` remains rejected until a separate final ordinary-process integration is reviewed and qualified.
...
The strict actuation binding is now implemented but remains non-reachable from ordinary incident handling. A short-lived capability carries a unique authorization id, incident id, exact `ProcessKey`, normalized image path/SHA-256, the protection snapshot observation time, and an explicit expiry. Revalidation rejects expiry/replay, PID reuse, image drift, protection transitions, degraded telemetry, critical/unknown process state, RansomGuard itself, and protected service processes. The current `GuardWorker` is source-gated against referencing the production state-change backend while `AutomaticContainment=false`.
...
The production containment candidate is `WindowsProcessStateChangeLease`. It retains one exact process handle, revalidates PID + process creation FILETIME, fresh image identity and critical-process state, creates a Windows process state-change object, and applies suspend/resume through that object. Normal completion must explicitly resume. The final state-change handle lifetime is the crash-release safety backstop: exact-SHA disposable-VM qualification proved that hard termination of the helper holding the final state-change handle released the target and its heartbeat resumed. Unsupported API availability fails closed; production code must not fall back to `SuspendThread`, `NtSuspendProcess`, kill, or process-tree intervention.
...
Before production containment actuation is enabled, the remaining chain is a separate ordinary-process integration:
...
No LAB fast-path threshold or `Eligible` authorization result may be promoted directly into a production blocking action. `AutomaticContainment=true` remains a separate final change followed by hosted and disposable-VM exact-SHA qualification.
