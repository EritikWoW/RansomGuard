...
Status: engineering threat model for RansomGuard 0.8.7 production-containment integration, covering the default Audit product, admitted Production Enforce lifecycle, protocol-v18 LAB/ProductionGate separation, hard-link/FSCTL policy, the Engineering minifilter, and the qualified crash-safe production containment path. The feature remains default-off until final exact-SHA integration qualification.
...
| Ordinary user-mode process inside an explicitly activated Production Enforce root | In scope for preserve-before-allow / fail-safe gate policy and, when explicitly configured and runtime-qualified, one bounded state-change containment session | Default remains `AutomaticContainment=false`; configured containment is actionable only while runtime publishes healthy Protected + `AutomaticContainmentActive` |
...
Audit mode stays non-blocking. Production Enforce may opt into detector-driven ordinary-process containment only with explicit `AutomaticContainment=true`; the shipped/default configuration remains false.

Authorization remains evidence-driven and fail-closed. After the incident archive is successfully created, the service evaluates an immutable containment-authorization snapshot and durably writes `authorization.json`. The decision is exposed through read-only runtime/incident diagnostics as exactly `Eligible`, `Denied`, or `DisabledByConfiguration` with explicit veto reasons. A configured request is not Eligible unless runtime protection is exactly Enforce/Protected and `AutomaticContainmentActive=true`.
...
The strict actuation binding is now wired only through `ProductionContainmentCoordinator`. A short-lived capability carries a unique authorization id, incident id, exact `ProcessKey`, normalized image path/SHA-256, the protection snapshot observation time, and an explicit expiry. Immediately before intervention, the coordinator opens the exact target through the qualified state-change backend and revalidates expiry/replay, PID reuse, fresh image identity, protection state, telemetry health, critical/unknown state, RansomGuard itself and protected service processes.

The older per-thread `ContainmentActuator` remains qualified engineering evidence for exact-thread identity, partial rollback, timeout/cancellation and restart-recovery behavior. It uses exactly one `SuspendThread` increment per exact thread instance and records ownership by `ThreadId + creation FILETIME`. It is **not** the production crash-safe primitive and must not be used as a fallback on systems where the qualified process state-change API is unavailable.

Production ordinary-process containment uses `WindowsProcessStateChangeLease`. It retains one exact process handle, revalidates PID + process creation FILETIME, fresh image identity and critical-process state, creates a Windows process state-change object, and applies suspend/resume through that object. `ProductionContainmentCoordinator` durably records `Prepared` before intervention, then `SuspendApplied`, performs only the bounded configured hold, explicitly resumes, and records `ExplicitResumeApplied` plus `Completed`. Cancellation/handled failure attempts explicit resume and records an abnormal terminal outcome. The final state-change handle lifetime is the crash-release safety backstop. Unsupported API availability, invalid journal state or incomplete prior sessions keep runtime automatic containment inactive; production code has no fallback to `SuspendThread`, `NtSuspendProcess`, kill, or process-tree intervention.

Exact-SHA disposable-VM run `36225304884` qualified source `8983455fde991a2a85682058a45ed9f8773fc03d`. Artifact `ransomguard-containment-actuator-8983455fde991a2a85682058a45ed9f8773fc03d` has digest `sha256:6f73539191cb6bd466a210fc2774c16ff81710f1e5ab2656958376bc57b14a26` and reported `passed=true` on `Microsoft Windows NT 10.0.28000.0`. The run proved exact identity/binding, unrelated-process isolation, replay/PID/image/lifecycle/telemetry/self/service vetoes, partial rollback, restart recovery, timeout/cancellation recovery, explicit state-change resume and hard-crash release. This evidence applies to that exact source and tested environment; it is not a broad Windows/Server compatibility claim.

The pre-enable operator recovery contract is documented in [CONTAINMENT_ACTUATOR_RECOVERY.md](CONTAINMENT_ACTUATOR_RECOVERY.md). After an unexpected service/helper exit, RansomGuard must not fabricate an explicit-resume record. Startup recovery preserves incomplete evidence, re-establishes current protection health and relies on the qualified final-handle crash-release property rather than guessing a thread/process resume primitive. If the exact target still appears non-progressing after handle loss, automatic admission stays disabled and recovery fails closed for operator/security review.

Final integration qualification must preserve this ordinary-process chain:

`incident evidence -> exact live identity -> preservation health -> authorization -> strict binding -> qualified state-change session -> bounded response -> explicit resume -> post-actuation audit`

No LAB fast-path threshold or bare `Eligible` authorization result may be promoted directly into a production action. The integration is acceptable only after hosted checks and disposable-VM exact-SHA qualification pass for the wired source; the default shipped configuration remains `AutomaticContainment=false`.
