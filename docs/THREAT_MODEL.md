...
| Clean transaction-complete GateClient requests DeactivateGate with zero gate/pending work | Kernel enters Maintenance; subsequent port close may clear retained root | Explicit authorized release rather than disconnect-as-disable |
...
A clean shutdown is a distinct maintenance transition. GateClient first requires a clean durable transaction/lifecycle state and then sends `DeactivateGate`. The kernel refuses this command while a blocking gate send or queued evidence work remains. Only a successful command clears the protection-required latch and reports Maintenance; the subsequent port close may clear the root.
