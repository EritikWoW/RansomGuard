...
The next foundation layer is still non-actuating: `ContainmentActuationLedger` durably consumes one authorization id into one immutable request, hash-chains every state transition with write-through flush semantics, and tracks suspension ownership by exact thread identity (`ThreadId + creation FILETIME`), not by a reusable TID alone. A resume record is accepted only for an increment that RansomGuard previously recorded as owned for that exact thread instance; completion refuses stranded increments. Partial failure can therefore be followed by durable owned-only recovery evidence without normalizing arbitrary pre-existing suspend counts. The ledger itself contains no Win32/NT suspension, resume or termination primitive and is not reachable from ordinary incident handling while `AutomaticContainment=false`.

The reversible actuator implementation is a separate, still-unwired production capability. It retains one exact process-instance lease before thread intervention, revalidates the short-lived binding immediately before actuation, enumerates a bounded thread set in bounded passes, and uses exactly one `SuspendThread` increment per exact thread instance. Every successful increment must be committed to the durable ownership ledger before another thread is touched. Any suspend/enumeration/timeout/cancellation failure transitions the request to an explicit failure and attempts owned-only `ResumeThread` compensation; recovery never calls `NtResumeProcess`, kill, process-tree termination, or suspend-count normalization. The Windows backend checks owner PID plus thread creation FILETIME before both suspend and resume. This implementation remains unreachable from `GuardWorker` until disposable-VM qualification proves the actuator and recovery contract.

The pre-enable operator recovery contract is documented in [CONTAINMENT_ACTUATOR_RECOVERY.md](CONTAINMENT_ACTUATOR_RECOVERY.md). Startup/operator recovery must verify the durable ledger before any new actuation admission, reopen only the exact recorded process instance, resume at most one increment for each outstanding exact TID + creation-FILETIME ownership record, append durable resume evidence immediately, and refuse to guess after identity or ledger validation failure. A process-exited terminal recovery outcome is not yet implemented; production automatic containment therefore remains disabled until that case and the full restart-recovery path are qualified.
