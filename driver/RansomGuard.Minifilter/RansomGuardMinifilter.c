...
static volatile LONG gProtectionClientMode = 0;
                           ...
                           PULONG Decision)
{
    ...
    clientMode = RgCurrentClientMode();
    if (!RgIsGateClientMode(clientMode)) {
        InterlockedDecrement(&gGateInFlight);
        if (ErrorCode != NULL) {
            *ErrorCode = (ULONG)STATUS_PORT_DISCONNECTED;
        }
        return FALSE;
    }

    if (RgAcquireClientPort(clientMode)) {
    ...
    if (clientMode == RgClientProductionGate && reply.Flags != 0) {
        return FALSE;
    }
    ...
    if (allow && clientMode == RgClientLabGate &&
...
static VOID RgQueueRawEvent(const RG_EVENT *Event, LONG ClientMode)
{
    ...
    if (Event == NULL || (ClientMode != RgClientAudit && !RgIsGateClientMode(ClientMode))) {
        return;
    }
    ...
    *ConnectionPortCookie = NULL;
    ...
    if (context->ProtocolVersion != RG_PROTOCOL_VERSION ||
        (context->ClientMode != RgClientAudit &&
         ...
         context->ClientMode != RgClientProductionGate)) {
    ...
    if (gClientPort != NULL || gContainedProcess != NULL ||
        ...
        if (InterlockedCompareExchange(&gProtectionRequired, 0, 0) == 0) {
            InterlockedExchange(&gProtectionClientMode, 0);
        ...
        InterlockedExchange(&gGateActivated, RgIsGateClientMode((LONG)context->ClientMode) ? 0 : 1);
...
static VOID RgDisconnect(PVOID ConnectionCookie)
{
    ...
    if (protectionRequired != 0 && gracefulDisconnect == 0) {
        ...
        // A replacement v18 GateClient may reconnect only with the retained client mode to this exact root/volume and rerun preflight.
    } else {
        InterlockedExchange(&gProtectionRequired, 0);
        InterlockedExchange(&gProtectionClientMode, 0);
        InterlockedExchange(&gDegradedProtected, 0);
