...
foreach($line in $infLines){
    ...
    if($trim -eq '; RansomGuard read-only minifilter LAB prototype.' -or
       $trim -eq '; IMPORTANT: 370099.4242 is an UNASSIGNED LAB PLACEHOLDER altitude.' -or
       $trim -eq '; Do not distribute/deploy this driver outside an isolated test VM.' -or
       $trim -eq '; Request a Microsoft-assigned altitude before any real deployment.'){
        continue
    }
    ...
    }elseif($trim.StartsWith('DiskId1', [StringComparison]::Ordinal)){
        $transformed.Add('DiskId1            = "RansomGuard Production Lifecycle Qualification Disk"')
    }elseif($trim.StartsWith('DefaultInstance', [StringComparison]::Ordinal)){
        $transformed.Add('DefaultInstance    = "RansomGuard Production Qualification Instance"')
    }elseif($trim.StartsWith('Instance1.Name', [StringComparison]::Ordinal)){
        $transformed.Add('Instance1.Name     = "RansomGuard Production Qualification Instance"')
    }elseif($trim.StartsWith('Instance1.Altitude', [StringComparison]::Ordinal)){
        $transformed.Add(('Instance1.Altitude = "{0}"' -f $QualificationAltitude))
    }else{
...
$expectedLines=@(
    ...
    'DiskId1            = "RansomGuard Production Lifecycle Qualification Disk"',
    'DefaultInstance    = "RansomGuard Production Qualification Instance"',
...
foreach($expected in $expectedLines){
    if(-not $transformed.Contains($expected)){
