...
if($proto -notmatch '#define\s+RG_PROTOCOL_VERSION\s+7u'){throw 'Minifilter protocol must be v7 for CREATE and rename completion reconciliation.'}
...
if($proto -notmatch 'DestinationPathStatus' -or $proto -notmatch 'DestinationPath\[RG_PATH_CHARS\]'){throw 'Protocol v7 must carry bounded reconciled name metadata.'}
if($proto -notmatch 'RgEventCreateResult' -or $proto -notmatch 'RgEventRenameResult' -or
   $proto -notmatch 'RelatedSequence' -or $proto -notmatch 'CompletionStatus'){
  throw 'Protocol v7 must carry correlated post-CREATE and post-rename completion metadata.'
}
...
Write-Host 'In-scope CREATE/WRITE/RENAME/DELETE/TRUNCATE require an explicit user-mode preservation decision; allowed CREATE/RENAME operations emit post-operation reconciliation.'
