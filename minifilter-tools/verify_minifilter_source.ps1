...
$driverHeader=Join-Path $sourceRoot 'driver\RansomGuard.Minifilter\RansomGuardMinifilter.h'
...
if(-not (Test-Path -LiteralPath $c) -or -not (Test-Path -LiteralPath $driverHeader) -or
   -not (Test-Path -LiteralPath $h) -or -not (Test-Path -LiteralPath $inf)){
    throw 'Minifilter source/header/protocol/INF missing.'
}
$src=Get-Content -LiteralPath $c -Raw
$driverHeaderText=Get-Content -LiteralPath $driverHeader -Raw
...
if($driverHeaderText -notmatch '(?m)^\s*#define\s+RG_MAX_GATE_INFLIGHT\s+8L\s*
    'RgClientLabGate',
...
Write-Warning 'The blocking gate is LAB ONLY. Do not load this prototype on a primary workstation.'
    ...
    throw 'Kernel gate header must cap blocking preservation at RG_MAX_GATE_INFLIGHT 8L.'
}
foreach($required in @(
    'RgClientLabGate',
    'RgEventIsInsideGateRoot',
...
Write-Warning 'The blocking gate is LAB ONLY. Do not load this prototype on a primary workstation.'
    ...
    throw 'Kernel gate admission cap must remain exactly 8 concurrent blocking requests.'
}
foreach($required in @(
    'RgClientLabGate',
    'RgEventIsInsideGateRoot',
