...
# The candidate binaries and immutable driver package remain exact-SHA frozen, while
# qualification harness changes may evolve on main. Bind evidence to that orchestration
# revision explicitly instead of silently executing an older harness from CandidateRoot.
$orchestrationRoot=[IO.Path]::GetFullPath((Split-Path -Parent $PSScriptRoot))
$runtimeIntegrationScript=Join-Path $orchestrationRoot 'minifilter-tools\run_runtime_integration_lab.ps1'
if(-not(Test-Path -LiteralPath $runtimeIntegrationScript -PathType Leaf)){
...
$summary=[ordered]@{
    ...
    orchestrationCommit=$orchestrationCommit
...
try{
    ...
    try{
        & $runtimeIntegrationScript -LabReleaseDirectory $lab -DriverPackageDirectory $driver -RootBase $root -ResultsDirectory $runtimeResults
        ...
        Require-True $runtime @(
            ...
            'gracefulReleaseSucceeded','mappedThenDisconnectBaselineVerified',
            'mappedThenDisconnectPagingObserved','mappedThenDisconnectPreimageHashMatched',
            'mappedThenDisconnectMutationObserved','mappedThenDisconnectDeniedMutation',
            'mappedThenDisconnectPreservedPostMapHash','mappedThenDisconnectReconnectActivated',
            'scopeAmbiguityDeniedMutation','scopeAmbiguityPreservedTargetHash',
