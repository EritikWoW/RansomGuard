...
Console.WriteLine("RansomGuard Minifilter AUDIT client v0.7.1.0");
...
{
    private long _events, _inScope, _write, _rename, _delete, _driverDropped, _resolved, _unresolved;
    ...
    public void Observe(RgEvent e, double lag, bool inScope, bool resolved)
    {
        ...
        switch ((RgEventType)e.EventType) { case RgEventType.Write: _write++; break; case RgEventType.Rename: _rename++; break; case RgEventType.DeleteDisposition: _delete++; break; }
    ...
    public string OneLine() => $"events={_events} scope={_inScope} write={_write} rename={_rename} delete={_delete} driverDropped={_driverDropped} unresolved={_unresolved} lag p50={P(.50):F1} p95={P(.95):F1} p99={P(.99):F1} max={(_lag.Count == 0 ? 0 : _lag.Max()):F1}ms";
    public object Snapshot(Options o, string logPath) => new
    {
        ...
        Events = _events, InScope = _inScope, Writes = _write, Renames = _rename, DeleteDispositions = _delete,
