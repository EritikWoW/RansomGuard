...
var createTransactionStore = new CreateTransactionStore(Path.Combine(store.Root, "create-operation-state"));
    ...
    while (!cts.IsCancellationRequested)
    {
        ...
        if ((RgEventType)ev.EventType == RgEventType.CreateResult)
        {
            var completion = await CreateReconciliation.HandleAsync(
                ev, resolver, options.Root, createTransactionStore, cts.Token).ConfigureAwait(false);
            if (completion is not null)
            {
                Console.WriteLine(
                    $"{DateTime.Now:HH:mm:ss.fff} {RgEventType.CreateResult,-20} request={ev.RelatedSequence,-7} {completion.State,-24} info={completion.CompletionInformation,-3} status=0x{completion.CompletionStatus:X8} {completion.FinalPath}");
            }
            continue;
        }
        ...
        var reply = await GateDecision.EvaluateAsync(ev, resolver, options.Root, store, writeStore, createStore,
            createTransactionStore, identityStore, renameStore, cts.Token);
...
static class CreateReconciliation
{
    public static async Task<CreateOperationCompletion?> HandleAsync(
        ...
        string root,
        CreateTransactionStore transactionStore,
        CancellationToken cancellationToken)
    {
        if (ev.ProtocolVersion != 7 || ev.RelatedSequence == 0)
            throw new InvalidDataException("Invalid CREATE completion correlation.");

        // Plain FILE_OPEN operations are intentionally not journaled; their result events are ignored.
        if (!transactionStore.TryGetIntent(ev.RelatedSequence, out _))
            return null;

        if (!NtSuccess(ev.CompletionStatus))
        {
            return await transactionStore.RecordCompletionAsync(
                    ev.RelatedSequence,
...
static class GateDecision
{
    public static async Task<RgGateReply> EvaluateAsync(RgEvent ev, DevicePathResolver resolver, string root,
        ...
        CreateTransactionStore createTransactionStore, FileIdentityStore identityStore,
        RenameRollbackStore renameStore, CancellationToken cancellationToken)
    {
        try
        {
            ...
            if (eventType == RgEventType.Create)
                return await EvaluateCreateAsync(ev, path, store, createStore, createTransactionStore,
                    identityStore, cancellationToken).ConfigureAwait(false);
        ...
        CancellationToken cancellationToken)
    {
        ...
        var trackCompletion = CreateGatePolicy.RequiresCompletionTracking(disposition, createOptions);
        var state = PathProbe.Get(path);
        var action = CreateGatePolicy.Decide(disposition, state, createOptions);
        if (action == CreatePreservationAction.DenyUnsupported)
            return Deny(ev.Sequence, 8);

        var originallyAbsent = createStore.TryGetBaseline(path, out var absenceBaseline);
        DurableFileIdentity? currentIdentity = null;

        if (state == CreateTargetState.File && trackCompletion)
        {
            var identityBaseline = await identityStore.CaptureOrVerifyAsync(path, cancellationToken)
                .ConfigureAwait(false);
            currentIdentity = identityBaseline.Identity;
        }

        var effectiveAction = action;
        var preservationHash = string.Empty;
        RgGateDecision decision;

        if (originallyAbsent)
        {
            // Pre-incident truth wins over the current incident-created bytes. Never manufacture
            // a pre-image from a file that did not exist before this incident.
            effectiveAction = CreatePreservationAction.RecordOriginallyAbsent;
            preservationHash = absenceBaseline?.RecordSha256
                ?? throw new InvalidDataException("Originally-absent CREATE baseline disappeared.");
            decision = RgGateDecision.BaselineCommitted;
        }
        else
        {
            switch (action)
            {
                case CreatePreservationAction.CaptureExistingPreimage:
                    ...
                    var capture = await store.CapturePreimageAsync(
                            path, RollbackMutationKind.Create, currentIdentity, cancellationToken)
                        .ConfigureAwait(false);
                    preservationHash = capture.RecordSha256;
                    decision = RgGateDecision.SnapshotCommitted;
                    break;

                case CreatePreservationAction.RecordOriginallyAbsent:
                    var baseline = await createStore.CaptureAbsentAsync(path, cancellationToken)
                        .ConfigureAwait(false);
                    originallyAbsent = true;
                    preservationHash = baseline.RecordSha256;
                    decision = RgGateDecision.BaselineCommitted;
                    break;

                default:
                    decision = RgGateDecision.NoPreservationRequired;
                    break;
            }
        }
        ...
        if (trackCompletion)
        {
            _ = await transactionStore.CaptureIntentAsync(
                    ev.Sequence,
                    path,
                    disposition,
                    createOptions,
        ...
        return Allow(ev.Sequence, decision);
    }
