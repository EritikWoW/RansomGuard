...
Console.WriteLine("RansomGuard LAB pre-write gate v0.7.1.0");
...
Console.WriteLine("Writes/rename/delete in this root are denied unless a durable pre-image commit succeeds.");
...
try
{
    while (!cts.IsCancellationRequested)
    {
        ...
        var reply = await GateDecision.EvaluateAsync(ev, resolver, options.Root, store, cts.Token);
...
static class GateDecision
{
    public static async Task<RgGateReply> EvaluateAsync(RgEvent ev, DevicePathResolver resolver, string root,
        RollbackStore store, CancellationToken cancellationToken)
    {
        try
        {
            if (ev.ProtocolVersion != 2 || (ev.PathStatus != (uint)RgPathStatus.Resolved && ev.PathStatus != (uint)RgPathStatus.Truncated))
            ...
            if (!File.Exists(path))
            {
                // v0.7.1 does not yet model file creation. Fail closed inside the explicit LAB root.
                return Deny(ev.Sequence, 3);
            }

            var mutation = (RgEventType)ev.EventType switch
            {
                RgEventType.Write => RollbackMutationKind.Write,
...
sealed record Options(string Root, string StoreRoot, string? SessionId, bool PrepareOnly)
{
    public static Options Parse(string[] args)
    {
        ...
        store ??= Path.Combine(Environment.GetFolderPath(Environment.SpecialFolder.LocalApplicationData), "RansomGuardV071", "GateRollback");
...
enum RgEventType : uint { Invalid = 0, Write = 1, Rename = 2, DeleteDisposition = 3 }
