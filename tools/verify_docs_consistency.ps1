...
foreach($required in @(
    ...
    @('Product target GateClient-loss fail-safe','0.7.32 hardens the activated GateClient-loss boundary',$target)
)){
...
Write-Host "Documentation consistency gate PASSED: operator docs, product target, README and changelog match version $version, protocol v15, 0.7.28 bounded concurrency, 0.7.29 fault campaigns, 0.7.30 Driver Verifier, 0.7.31 mixed endurance and the 0.7.32 GateClient-loss fail-safe milestone."
