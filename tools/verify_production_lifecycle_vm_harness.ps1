...
foreach($required in @(
    ...
    if($prepare.IndexOf($required,[StringComparison]::Ordinal) -lt 0){
...
foreach($required in @(
    ...
    "$_.PSObject.Properties['Session']",
    "$_.PSObject.Properties['Event']",
    "'Event:'+[string]$eventProperty.Value",
    "Wait-AuditType 'RollbackStoreReady'",
    ...
    'serviceFailureDiagnosticsCaptured',
    'production-lifecycle-service-diagnostics.json',
    'Production package was not admitted before lifecycle startup.',
    "'DegradedProtected'",
