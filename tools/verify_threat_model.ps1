...
foreach($required in @(
    ...
    'tracks suspension ownership by exact thread identity (`ThreadId + creation FILETIME`)',
    'The reversible actuator implementation is a separate, still-unwired production capability.',
    'uses exactly one `SuspendThread` increment per exact thread instance.',
    'recovery never calls `NtResumeProcess`, kill, process-tree termination, or suspend-count normalization.',
    'The Windows backend checks owner PID plus thread creation FILETIME before both suspend and resume.',
    'This implementation remains unreachable from `GuardWorker` until disposable-VM qualification proves the actuator and recovery contract.',
