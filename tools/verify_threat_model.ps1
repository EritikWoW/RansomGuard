...
foreach($required in @(
    ...
    'Authorization remains evidence-driven and fail-closed.',
    'A configured request is not Eligible unless runtime protection is exactly Enforce/Protected and `AutomaticContainmentActive=true`.',
    'The strict actuation binding is now wired only through `ProductionContainmentCoordinator`.',
    'Immediately before intervention, the coordinator opens the exact target through the qualified state-change backend',
    ...
    'Production ordinary-process containment uses `WindowsProcessStateChangeLease`.',
    'The final state-change handle lifetime is the crash-release safety backstop',
    'Unsupported API availability, invalid journal state or incomplete prior sessions keep runtime automatic containment inactive',
    ...
    '`ProductionContainmentCoordinator` durably records `Prepared` before intervention'
    'After an unexpected service/helper exit, RansomGuard must not fabricate an explicit-resume record.',
    'No LAB fast-path threshold or bare `Eligible` authorization result may be promoted directly into a production action.'
