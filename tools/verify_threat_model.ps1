...
foreach($required in @(
    ...
    'Protocol v18 has separate `Audit`, `LabGate` and `ProductionGate` client identities',
    'unexpected ProductionGate disconnect cannot reconnect as LabGate',
    'Hosted source/compile results alone are not runtime evidence'
)){
...
foreach($required in @(
    'minifilter/GateClient wire contract is protocol v18',
    ...
    'minifilter/GateClient wire contract is protocol v18',
    'ProductionGate cannot use LAB containment reply flags',
    'exact client mode across degraded reconnect',
    'CODEOWNERS',
...
foreach($required in @(
    ...
    'clientMode == RgClientProductionGate && reply.Flags != 0',
    'gDegradedProtected',
...
if(-not $protocol.Contains('#define RG_PROTOCOL_VERSION 18u') -or
   -not $protocol.Contains('RgClientProductionGate = 3')){
    throw 'Threat-model protocol-v18 ProductionGate boundary changed without review.'
}
...
if(-not $gateClient.Contains('public const uint Version = 18') -or
   -not $gateClient.Contains('ClientMode = (uint)(options.Production ? RgClientMode.ProductionGate : RgClientMode.LabGate)') -or
   ...
   -not $gateClient.Contains('DevicePathResolver.ToNtScope(options.Root)')){
    throw 'GateClient protocol-v18 ProductionGate/protected-volume connection boundary changed without threat-model review.'
}
foreach($required in @(
